...
best_ts_model <- accuracy_test_ts %>%
  ...
  filter(!is_flat | .model %in% c("drift", "tslm_trend", "arima_drift")) %>%
  group_by(island) %>%
...
fc_trend <- fc_test_ts %>%
  filter(.model %in% c("drift", "arima_drift", "tslm_trend"))
...
p_cor_ods <- cor_canarias |>
  ...
  ggplot(aes(x = reorder(indicator, spearman), y = spearman)) +
  geom_col() +
  coord_flip() +
  theme_minimal() +
  labs(
    title = "Correlación entre indicadores ODS y temperatura media en Canarias",
